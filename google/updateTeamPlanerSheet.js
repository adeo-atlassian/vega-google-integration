...
/*
below two methods are for one update at a time
...
async function factorVegaDate(prepData){
  ...
  for(let k in keys){
      let employee = keys[k]
      let employeeIndexInSheet = indices["employeeIndex"][employee] //index of employee row 

      // console.log(`Employee: ${employee}, index: ${employeeIndexInSheet}`)

      if(employeeIndexInSheet == undefined){
        console.log(`No valid rows found for engineer '${employee}' in sheet` )
        continue
      }

      // console.log(`employeeIndexInSheet: ${JSON.stringify(indices["employeeIndex"])}`)
      ...
      for(let m = 0; m < employeeEvents.length; m++){
          try{
              ...
              let range = columnIndexToRange(sheetName, startEventIndex, endEventIndex, employeeIndexInSheet)
            ...
            if(isAfter(eventEndDate, endDate)){
              ...
              for(let c = 0; c < diffDays; c++){
                  ...
                  let searchHash = `${employeeIndexInSheet}#${currentCol}`
                  let searchValue = existingDataInSheet[searchHash]
                  // console.log(`Search hash: ${eventStartDate}: ${range} ${employeeIndexInSheet}:${currentCol}, sheet val: ${searchValue}, vega value: ${eventType}`)

                  if(searchValue != undefined && searchValue != eventType){

                    console.log(`conflict or multi event:@${employee} ${employeeIndexInSheet}: ${eventStartDate}(+)${c}, sheet val: ${searchValue}, vega value: ${eventType}`)
                    ...
                    if(searchValue != undefined){
                      valueArray.push(searchValue)
                      syncedData[searchHash] = searchValue
                    }else{
                      valueArray.push(eventType)
                      syncedData[searchHash] = eventType
                    }
    ...
    for(let eKey in existingDataInSheet){
    if(sheetDataConflict[eKey] == undefined && syncedData[eKey] == undefined){
      // console.log(`Existing key: ${eKey} value: ${existingDataInSheet[eKey]}`)
      dataToPushToVega[eKey] = existingDataInSheet[eKey]
    }
    
    }
    // console.log(`dataToPushToVega : ${JSON.stringify(dataToPushToVega)}`)
...
async function main(prepData) {
    //setEnvVariables() call is mandatory for updating gshseets
    setEnvVariables(); 

    // primary function to prepare and push data to gsheets
